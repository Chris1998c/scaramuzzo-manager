...
export default function InventarioPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      fetchProducts(s, filter, category);
    }
  ...
  // ============================
  // FETCH PRODOTTI (sempre da salon globale)
  // ============================
  ...
  return (
    <div className="px-6 py-10 bg-[#1A0F0A] min-h-screen text-white">
      ...
      <p className="text-white/60 mb-6">
        Visualizzazione sincronizzata con il selettore salone in alto.
      </p>
            ...
            {products.map((p) => (
              <tr key={p.id} className="border-b">
                ...
                <td className="p-3 text-right space-x-3">
                  <Link
                    href={`/dashboard/magazzino/carico?product=${p.id}`}
                  ...
                  <Link
                    href={`/dashboard/magazzino/scarico?product=${p.id}`}
                  ...
                  {(role === "magazzino" || role === "coordinator") && (
                    <>
                      <Link
                        href={`/dashboard/magazzino/prodotto/${p.id}/modifica`}
                      ...
                      <Link
                        href={`/dashboard/magazzino/prodotto/${p.id}/qr`}
