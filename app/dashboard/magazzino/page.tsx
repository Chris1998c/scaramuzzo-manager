...
export default function MagazzinoPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!res.ok) {
          console.error("Errore /api/auth/me", await res.text());
          return;
        }
        ...
        console.log("ROLE LETTO →", r);
        console.log("SALON ID LETTO →", s);
  ...
  // ====================================
  // COMPONENTE CARD PREMIUM (Apple look)
  // ====================================
  ...
  return (
    <div className="min-h-screen px-6 py-10 bg-[#1A0F0A] text-[#FDF8F3]">
      ...
      {/* TITOLO */}
      <h1 className="text-4xl font-extrabold mb-3 text-[#B88A54]">
        Magazzino
      </h1>
      ...
      {/* ============================================
          PULSANTE NUOVO PRODOTTO (solo magazzino+coord)
          ============================================ */}
