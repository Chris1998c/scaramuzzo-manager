...
export default function AgendaModal({
 ...
 async function createAppointment() {
  ...
  try {
    ...
    const apptStart = toNoZ(cursor);
    const apptEnd = toNoZ(new Date(cursor.getTime() + total * 60_000));

    // 1) creo UN SOLO appuntamento
    const { data: appt, error: apptErr } = await supabase
      .from("appointments")
      .insert({
        salon_id: Number(activeSalonId),
        customer_id: customerId, // UUID string
        staff_id: selectedSlot.staffId ? Number(selectedSlot.staffId) : null, // staff_id = int nel tuo DB
    ...
    // 2) righe appointment_services (NO end_time: non esiste nello schema)
const rows = selectedServiceIds.map((sid) => {
  const s = services.find((x: any) => Number(x.id) === Number(sid));
  const dur = Math.max(15, Number(s?.duration ?? 15));
  const price = Number(s?.price ?? 0);

  // ✅ VAT: se non hai la colonna sul service, metti default 22
  const vatRate = Number((s as any)?.vat_rate ?? 22);

  const segStart = new Date(cursor);
  cursor = new Date(cursor.getTime() + dur * 60_000);

  return {
    appointment_id: Number(appointmentId),
    service_id: Number(sid),
    duration_minutes: dur,
    start_time: toNoZ(segStart),
    staff_id: serviceAssignments[sid] ? Number(serviceAssignments[sid]) : null,
    price,
    vat_rate: vatRate, // ✅ OBBLIGATORIO
  };
});



    const { error: segErr } = await supabase.from("appointment_services").insert(rows);
    ...
    console.error(e);
    setErr("Errore durante il salvataggio: " + (e?.message || "unknown"));
  } finally {
    setSaving(false);
  }
}
              ...
              {filteredCustomers.length > 0 ? (
                filteredCustomers.slice(0, 20).map(c => (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all border ${customerId === c.id
            ...
            <div className="grid gap-3">
              {services.map(s => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`group p-4 rounded-[1.5rem] border transition-all duration-300 ${isActive
