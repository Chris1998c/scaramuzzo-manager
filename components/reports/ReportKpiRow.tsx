...
export default function ReportKpiRow({ totals }: { totals: Totals }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-scz-dark p-6 md:p-8">
      ...
      >
        <div className={label}>Incassato</div>
        <div className={`${valuePrimary} mt-1 text-scz-gold`}>
          {money(totals.gross_total)}
        </div>
        <div className="mt-1 text-xs text-white/40">Periodo selezionato</div>
      </div>

      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className={card}>
          <div className={label}>Scontrini</div>
          <div className={`${valueSecondary} mt-1`}>{totals.receipts_count}</div>
        </div>
        <div className={card}>
          <div className={label}>Imponibile</div>
          <div className={`${valueSecondary} mt-1`}>{money(totals.net_total)}</div>
        </div>
        <div className={card}>
          <div className={label}>IVA</div>
          <div className={`${valueSecondary} mt-1`}>{money(totals.vat_total)}</div>
        </div>
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className={card}>
          <div className={label}>Sconti dati</div>
          <div className={`${valueSecondary} mt-1`}>{money(totals.discount_total)}</div>
        </div>
        <div className={card}>
          <div className={label}>Servizi</div>
          <div className={`${valueSecondary} mt-1`}>{money(totals.gross_services)}</div>
        </div>
        <div className={card}>
          <div className={label}>Prodotti</div>
          <div className={`${valueSecondary} mt-1`}>{money(totals.gross_products)}</div>
        </div>
      </div>
    </div>
  );
